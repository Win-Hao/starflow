...
复刻 OpenAI「GPT-6 Astra」发布页的星星粒子系统：一座会流动的螺旋星系，随滚动翻转、散成两侧星轨，
再聚成光标和 OpenAI 结。渲染、动画、交互全部用 three.js + postprocessing 重写，星星的分布、流动、
辉光和镜头光晕都按原站的算法逐项还原，同一批星星还能摆成任意文字和图标。
...
文案有中英两套（`src/i18n.js`），元素上写 `data-i18n="键"` 即可；默认按浏览器语言，切换后记在 `localStorage`。

![英文版](docs/screenshots/07-english.jpeg)
...
原站抓包的页面和反压缩后的分析笔记不在仓库里；星星相关的算法都已按上表在源码里注明出处。
