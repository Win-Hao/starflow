...
- `USAGE.md`, `components.html` (hero chrome, pills, copy column, shape cue, benchmarks, panels), `preview/{colors,typography,spacing}.html`
...
`components.manifest.json`, `design-tokens.json`, `tailwind-v4.css` are derived outputs; I left them for the maintainers' generation scripts to avoid drift-guard failures. Happy to add them if you prefer they ship in the PR.

## Checks

- `pnpm typecheck`
- `pnpm guard` (manifest + token contract)
